...
const main = async function () {
  ...
  const update_last_pull = function () {
    queue.update_last_pull({ originator: address, process_uuid }).catch(() => {
      logger.error("Database error when updating last pull epoch");
    });
  };
  ...
  const save_state_async = function (ids, state) {
    queue.save_state(ids, state).catch((err) => {
      logger.error(
        `Database error when setting ${state} on operation with ids: ${JSON.stringify(
          ids
        )}`
      );
    });
  ...
  const health_check = async function () {
    ...
    try {
      ...
      logger.info(
        `An error has occurred while attempting to get tez balance; the node may be down or inaccessible.\n${err}`
      );
      return false;
    }
    ...
    if (tez_supply > config.warnBelowTez) {
      ...
      await queue.add_balance_warning({
        originator: address,
        process_uuid,
        tez_supply,
      });
  ...
  const heartbeat = async function () {
    ...
    let counts = Object.values(queue.state)
      .map((state) => {
        const count =
          state_counts.rows.find((r) => r.state == state)?.count || 0;
        return `${count} ${state.toUpperCase()}`;
      })
      .join(", ");
    logger.info(`Originator ${address} has ${counts} operations`);

    // check distinct op hashes
    let distinct_ophashes = await queue.get_distinct_ophash(address);
    for (let { ophash } of distinct_ophashes.rows) {
      logger.info(`Checking op hash ${ophash}`);
      try {
        let tzkt_response = (
          await axios.get(
            `${config.tzktUrl}/v1/operations/transactions/${ophash}`,
            {
              timeout: 3000,
            }
          )
        ...
        if (is_applied) {
          logger.info(`op hash ${ophash} is applied`);
        } else {
          logger.error(
    ...
    let ops = await queue.checkout(
      address,
      ~~(config.batchSize / batch_divider) + 1
    );
    if (ops.length == 0) {
    ...
    if (rejected_ids.length > 0) {
      logger.warn(
        `Rejected operations with ids: ${JSON.stringify(rejected_ids)}`
      );
    ...
    logger.info(
      `Attempting to send operation group containing operations with ids: ${JSON.stringify(
        batched_ids
      )}`
    );
    try {
      ...
      logger.info(
        `Sent operation group with hash ${op_hash} containing operations with ids: ${JSON.stringify(
          batched_ids
        )}`
      );
      ...
      queue.save_sent(batched_ids, sent_operation.opHash).catch((err) => {
        logger.error(
          `Database error when writing hash ${
            sent_operation.opHash
          } to sent operations with ids: ${JSON.stringify(batched_ids)}`
        );
      });

      // Wait for it to be baked
      let result = await sent_operation.confirmation(
        config.confirmations,
        config.timeout
      );
      if (result.completed) {
        logger.info(
          `Operation group with hash ${op_hash} has been successfully confirmed.`
        );
        ...
        logger.error(
          `Operation group with hash ${op_hash} has failed on chain.`
        );
      ...
      if (tezos_error) {
        switch (tezos_error.id_noproto) {
          ...
          case "storage_exhausted.operation":
            ...
            logger.warn(
              `Retriable Tezos error encountered:\n${tezos_error}\nRetrying operations with ids: ${JSON.stringify(
                batched_ids
              )}`
            );
      ...
      logger.error(
        `An unhandled error has occurred when processing operations with ids: ${JSON.stringify(
          batched_ids
...
main()
  ...
  .catch((err) => {
    logger.info(`An error has ocurred outside the main event loop.\n${err}`);
  });
