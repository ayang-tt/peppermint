...
export default async function (tezos, { contract_address }) {
  ...
  logger.info(
    `token contract loaded ${nft_contract.parameterSchema.ExtractSignatures()}`
  );
  ...
  let create_token = function (token_id, metadata_ipfs) {
    ...
    let token_info = MichelsonMap.fromLiteral({
  ...
  let mint_token = function (token_id, to_address, amount = 1) {
    ...
    let mint_op = contract_ops.mint_tokens([
  ...
  let burn_token = function (token_id, from_address, amount = 1) {
    ...
    let burn_op = contract_ops.burn_tokens([
      { owner: from_address, token_id, amount },
    ]);
    return burn_op;
  };
  ...
  return {
    ...
    create_and_mint: function (
      { token_id, to_address, metadata_ipfs, amount },
      batch
    ) {
      let create_op = create_token(token_id, metadata_ipfs);
      let mint_op = mint_token(token_id, to_address, amount);
    ...
    create_and_mint_multiple: function (
      { token_id, metadata_ipfs, destinations },
      batch
    ) {
      let create_op = create_token(token_id, metadata_ipfs);
      let mint_args = destinations.map((e) => ({
        token_id,
        owner: e.to_address,
        amount: e.amount,
      }));
